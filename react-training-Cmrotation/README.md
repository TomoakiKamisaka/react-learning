...
CM（コマーシャル）の広告枠に広告主をドラッグ&ドロップで割り当てる「CM広告枠管理システム」です。

- **広告主検索**: 検索バーに入力した文字列で、配置可能な広告主一覧を絞り込む
- **広告主プール**: まだどの枠にも配置していない広告主をタグ状のボタンとして一覧表示
- **番組スケジュールグリッド**: 番組ごとに6枠のCM枠を表示し、広告主プールからドラッグ&ドロップすると該当枠に広告主名が表示され、プールから除外される

Claude Codeでこのプロジェクトを扱う場合は [`CLAUDE.md`](CLAUDE.md) も参照してください（コマンド一覧・コーディング規約をまとめています）。
